...
| Pydantic AI | 类型化 Python agent、结构化输出、显式工具与依赖、可观测性与持久执行 |
| Agno | 一体化的 agents、teams、workflows、memory/knowledge 与运行时管理 |
...
|--------------------|----------------|
| 模型与工具组合 | LangChain |
| 类型化 Python agent、结构化输出与显式工具设计 | Pydantic AI |
| 一体化 agent 栈与运行时管理 | Agno |
...
如果第一层判断结果是 `Agno`，下一层细分可以看：

- [agno-selection](./.agents/skills/agno-selection/SKILL.md)

如果第一层判断结果是 `Pydantic AI`，建议从这里开始：

- [pydanticai-dependencies](./.agents/skills/pydanticai-dependencies/SKILL.md)：包、provider 与安装选择
- [pydanticai-fundamentals](./.agents/skills/pydanticai-fundamentals/SKILL.md)：默认的第一条构建路径
...
未来可能会加入的框架包括 AutoGen 等，但这里提到它们只是说明扩展方向，不代表已经承诺全部覆盖。
